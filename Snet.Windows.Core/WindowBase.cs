...
{
    /// <summary>
    /// 自定义窗口基类，继承自 System.Windows.Window。<br/>
    /// 提供以下基础功能：<br/>
    /// - 皮肤（主题）切换与持久化<br/>
    /// - 多语言切换与持久化<br/>
    /// - 窗口控制按钮（最小化、最大化/还原、关闭）<br/>
    /// - DPI 感知与自动缩放适配<br/>
    /// - Win32 消息处理（最大化尺寸限制、吸附检测）<br/>
    /// - 原生背景色同步（消除拖拽/最大化时的白色闪烁）<br/>
    /// - 加载动画（淡入淡出过渡效果）
    /// </summary>
    public class WindowBase : System.Windows.Window
    {
        ...
        /// 窗体加载完成事件处理。<br/>
        /// 执行以下初始化操作：<br/>
        /// 1. 根据资源中的圆角半径值统一设置各区域圆角<br/>
        /// 2. 修改注册表优化窗口拖拽显示效果<br/>
        /// 3. 更新原生背景色（消除拖拽/最大化时的白色闪烁）<br/>
        /// 4. 执行自动缩放逻辑以适配当前屏幕
        /// </summary>
        ...
        /// <summary>
        /// 窗口抖动效果（用于提示用户注意，如输入错误时）。<br/>
        /// 通过水平方向的双向动画实现窗口抖动，重复 3 次后自动停止。
        /// </summary>
        /// <param name="window">要抖动的窗口；若为 null 则自动查找当前活动窗口</param>
